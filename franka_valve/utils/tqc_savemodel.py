...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def learn(
        ...
        print(self.num_timesteps, "training end")
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            for idx, done in enumerate(dones):
                ...
                if done:
                    ...
                    # if not infos[0]["collision"] and not infos[0]["bound"]:
                    #     label = "success_" + str(int(self.num_timesteps))
                    #     self.save(self.save_path.format(label))
                    if self.save_flag:
                        label = str(int(self.num_timesteps))
                        self.save(self.save_path.format(label))
                        self.save_flag = False
                    # Update stats
...
class TQCsm(OffPolicyAlgorithm):
    """
    ...
    def _excluded_save_params(self) -> List[str]:
        # Exclude aliases
        return super(TQCsm, self)._excluded_save_params() + ["actor", "critic", "critic_target"]
...
class TQCsm_new(OffPolicyAlgorithm):
    """
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            # Compute actor loss
            qf_pi = self.critic(replay_data.observations, actions_pi).mean(dim=2).mean(dim=1, keepdim=True)
            # actor_loss = (ent_coef * log_prob - qf_pi).mean() - 1 * replay_data.similarities.mean()#gradient acent
            # epsilon = np.clip((1-0.00001*self._episode_num), 0.01, 1)

            # actor_loss = (ent_coef * log_prob - qf_pi - replay_data.similarities).mean()  # gradient acent
            actor_loss = (ent_coef * log_prob - qf_pi).mean()
            self._episode_num
            actor_losses.append(actor_loss.item())
    ...
    def learn(
        ...
        return super(TQCsm_new, self).learn(
            total_timesteps=total_timesteps,
            callback=callback,
            log_interval=log_interval,
            eval_env=eval_env,
    ...
    def _excluded_save_params(self) -> List[str]:
        # Exclude aliases
        return super(TQCsm_new, self)._excluded_save_params() + ["actor", "critic", "critic_target"]
